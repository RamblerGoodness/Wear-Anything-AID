...
function WTG_Output(text) {
  ...
  if (state.pendingCharacterEntries) {
    ...
    if (lastAction) {
      ...
      const shouldPopulateEntry = (card) => {
        if (!card) {
          return false;
        }
        ...
        if (entry.startsWith("Discovered in:")) {
          return true;
        }
        const desc = card.description || "";
        return desc.includes("Generated character") || desc.includes("[AI-GENERATED]");
      };
      ...
      for (const name in state.pendingCharacterEntries) {
        if (state.pendingCharacterEntries[name]) {
          ...
          if (card && shouldPopulateEntry(card)) {
  ...
  // entity detection from the original AI output with parentheses intact.
  {
    ...
    const findExistingCard = (title) => storyCards.find(card =>
      card.title && card.title.toLowerCase() === title.toLowerCase()
    );
    ...
    if (generatedEntities.length > 1) {
      ...
      for (const entity of generatedEntities) {
        ...
        const existingCard = findExistingCard(entity.name);
        if (existingCard) {
          if (!hasTimestamp(existingCard)) {
            addTimestampToCard(existingCard, `${state.currentDate} ${state.currentTime}`, true);
          }
          continue;
        }
