...
// Optional AutoCards integration (no-op when AutoCards is unavailable).
function applyAutoCardsInput(text) {
  if (!text) {
    return text;
  }
  ...
  if (typeof isCommandText === "function" && isCommandText(text)) {
    return text;
  }
  return AutoCards("input", text);
}

function applyAutoCardsContext(text, stopVal) {
  if (typeof AutoCards !== "function") {
    return [text, stopVal];
  }
  const result = AutoCards("context", text, stopVal);
  if (Array.isArray(result)) {
    return result;
  }
  if (result && typeof result === "object") {
    return [result.text ?? text, result.stop ?? stopVal];
  }
  return [text, stopVal];
}

function applyAutoCardsOutput(text) {
  if (!text) {
    return text;
  }
  if (typeof AutoCards !== "function") {
...
function initCI() {
  ...
  if (ci.autoSyncOutfits === undefined) {
    ci.autoSyncOutfits = true;
  }
  if (ci.autoSyncOutfitsToCards === undefined) {
    ci.autoSyncOutfitsToCards = true;
  }
  if (ci.autoSyncOutfitsFromCards === undefined) {
    ci.autoSyncOutfitsFromCards = true;
  }
...
function createSettingsCard() {
  if (!storyCards.find(sc => sc.title === "CI Settings")) {
    ...
    settingsSC.description = [
      ...
      "",
      "Auto-Cards:",
      "[CI Outfit] blocks on character cards sync with outfits.",
      "Edit the block to update that character's outfit."
    ].join("\n");
  }
}
...
function storeSettingsToSC() {
  ...
  const preserved = existingLines.filter(line => {
    return !line.match(/^\s*(enabled|injectToAN|focusMaxEntries|autoSyncOutfits|autoSyncOutfitsToCards|autoSyncOutfitsFromCards|timeNoTime|timeSmall|timeMedium|timeLarge)\s*=/i);
  }).filter(line => line.trim() !== "");
  settingsSC.entry = [
    `enabled = ${String(ci.enabled)}`,
    `injectToAN = ${String(ci.injectToAN)}`,
    `autoSyncOutfits = ${String(ci.autoSyncOutfits)}`,
    `autoSyncOutfitsToCards = ${String(ci.autoSyncOutfitsToCards)}`,
    `autoSyncOutfitsFromCards = ${String(ci.autoSyncOutfitsFromCards)}`,
...
function loadSettingsFromSC() {
  ...
  const autoSyncMatch = settingsSC.entry.match(/autoSyncOutfits\s*=\s*(true|false)/i);
  if (autoSyncMatch) {
    ci.autoSyncOutfits = autoSyncMatch[1].toLowerCase() === "true";
  }
  const autoSyncToMatch = settingsSC.entry.match(/autoSyncOutfitsToCards\s*=\s*(true|false)/i);
  if (autoSyncToMatch) {
    ci.autoSyncOutfitsToCards = autoSyncToMatch[1].toLowerCase() === "true";
  }
  const autoSyncFromMatch = settingsSC.entry.match(/autoSyncOutfitsFromCards\s*=\s*(true|false)/i);
  if (autoSyncFromMatch) {
    ci.autoSyncOutfitsFromCards = autoSyncFromMatch[1].toLowerCase() === "true";
  }
...
function isCommandText(text) {
  ...
  return /^\s*(?:-?\s*>\s*)?(?:You\s+|I\s+)?\/(?:wear|takeoff|undress|reloadoutfit|outfit|remove|saveoutfit|loadoutfit|deleteoutfit)\b/i.test(text);
}
...
const CI_OUTFIT_BLOCK_START = "[CI Outfit]";
const CI_OUTFIT_BLOCK_END = "[/CI Outfit]";

function buildOutfitBlock(outfit) {
  return [CI_OUTFIT_BLOCK_START, buildOutfitEntry(outfit || {}), CI_OUTFIT_BLOCK_END].join("\n");
}

function extractOutfitBlock(entry) {
  if (!entry) {
    return null;
  }
  const startIndex = entry.indexOf(CI_OUTFIT_BLOCK_START);
  const endIndex = entry.indexOf(CI_OUTFIT_BLOCK_END);
  if (startIndex === -1 || endIndex === -1 || endIndex <= startIndex) {
    return null;
  }
  const start = startIndex + CI_OUTFIT_BLOCK_START.length;
  return entry.slice(start, endIndex).trim();
}

function upsertOutfitBlock(entry, block) {
  if (!entry || !entry.trim()) {
    return block;
  }
  const startIndex = entry.indexOf(CI_OUTFIT_BLOCK_START);
  const endIndex = entry.indexOf(CI_OUTFIT_BLOCK_END);
  if (startIndex !== -1 && endIndex !== -1 && endIndex > startIndex) {
    const afterEnd = endIndex + CI_OUTFIT_BLOCK_END.length;
    return `${entry.slice(0, startIndex).trimEnd()}\n${block}\n${entry.slice(afterEnd).trimStart()}`.trim();
  }
  return `${entry.trim()}\n\n${block}`.trim();
}

function isLikelyCharacterCard(card) {
  if (!card || !card.title) {
    return false;
  }
  if (card.type && card.type.toLowerCase() === "character") {
    return true;
  }
  if (card.description && card.description.toLowerCase().includes("character")) {
    return true;
  }
  return false;
}

function ensureUserOutfitState(name) {
  if (!name) {
    return;
  }
  addUser(name);
  const outfitCardTitle = `${name}'s Outfit`;
  let outfitCard = storyCards.find(sc => sc.title === outfitCardTitle);
  if (!outfitCard) {
    addStoryCard(outfitCardTitle, "Blank", "Outfit System");
    outfitCard = storyCards.find(sc => sc.title === outfitCardTitle);
    if (outfitCard) {
      outfitCard.keys = `Outfit story card for ${name}. Edit the entry to change items.`;
      outfitCard.description = [
        "Format:",
        "Category: item1, item2",
        "",
        "Use Empty to show a category with no items.",
      ...
      outfitCard.entry = "Empty";
    }
  }
  if (!outfitCard) {
    return;
  }
  const entry = (outfitCard.entry || "").trim();
  if (entry && entry.toLowerCase() !== "empty") {
    state.ci.users[name].outfit = parseOutfitEntry(entry);
    return;
  }
  if (!state.ci.users[name].outfit || Object.keys(state.ci.users[name].outfit).length === 0) {
    const defaultOutfit = getDefaultOutfitFromCard();
    if (defaultOutfit) {
      state.ci.users[name].outfit = cloneOutfit(defaultOutfit);
    }
  }
  outfitCard.entry = buildOutfitEntry(state.ci.users[name].outfit || {});
}

function syncCharacterOutfitsWithCards() {
  const ci = state.ci || {};
  if (!ci.autoSyncOutfits) {
    return;
  }
  const syncFrom = ci.autoSyncOutfitsFromCards;
  const syncTo = ci.autoSyncOutfitsToCards;
  if (!syncFrom && !syncTo) {
    return;
  }
  storyCards.forEach(card => {
    if (!isLikelyCharacterCard(card)) {
      return;
    }
    ...
    ensureUserOutfitState(name);
    if (syncFrom && card.entry) {
      const block = extractOutfitBlock(card.entry);
      if (block) {
        const parsed = parseOutfitEntry(block);
        state.ci.users[name].outfit = parsed;
      }
    }
    if (syncTo) {
      const block = buildOutfitBlock(state.ci.users[name].outfit || {});
      card.entry = upsertOutfitBlock(card.entry || "", block);
    }
  });
}
...
// --- AutoCards (LewdLeah Localized-Languages) ---
    ...
    if (!HOOK) {
        ...
        function getConfigureCardEntry() {
            return prose(
                ...
                "> Note: Automatic card generation is disabled by default here (cooldown set to 9999). Use Auto-Cards commands to generate cards manually, or lower the cooldown to enable periodic generation.",
